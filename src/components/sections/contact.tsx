...
export default function InvertedContact() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Email line underline animation
      if (emailRef.current) {
        gsap.fromTo(
          emailRef.current,
      ...
      <div className="mx-auto max-w-[720px]">
        ...
        <ScrollReveal delay={0.2}>
          <a
            ...
            className="inline-block text-[clamp(2rem,4vw,4rem)] font-medium tracking-[-0.02em] text-foreground hover:text-text-3 transition-colors"
            style={{
              backgroundImage:
          ...
          <div className="mt-16 flex flex-wrap gap-x-8 gap-y-2">
            {[
              ...
              <Link
                ...
                className="text-text-3 text-sm hover:text-foreground transition-colors"
              >
                {link.label}
              </Link>
            ))}
