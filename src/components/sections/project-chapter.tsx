...
export default function ProjectChapter({
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (imageRef.current) {
        ...
        // Image mask reveal — clips from bottom as it enters viewport
        gsap.fromTo(
          imageRef.current,
      ...
      // Chapter number scale animation
  ...
  const imageElement = (
      <div ref={imageRef} className="relative overflow-hidden" data-cursor="project">
  ...
  const textElement = (
    <div ref={textRef} className="flex flex-col justify-center">
      <span className="font-mono text-[0.7rem] font-semibold uppercase tracking-[0.12em] text-text-4 mb-4">
        Chapter {number}
      </span>
      <h2
        ref={titleRef}
      ...
      <Link
        href={href}
        className="group inline-flex items-center gap-2 text-sm text-text-2 transition-colors hover:text-text-1"
      >
        <span>View</span>
