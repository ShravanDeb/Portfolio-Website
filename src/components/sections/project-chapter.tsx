...
export default function ProjectChapter({
  ...
  const textElement = (
    <div ref={textRef} className="flex flex-col justify-center group/text">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="font-mono text-[0.6rem] md:text-[0.6rem] uppercase tracking-[0.15em] text-text-3 opacity-100 md:opacity-0 md:group-hover/text:opacity-100 transition-opacity duration-300 border border-border px-1.5 py-0.5 rounded bg-surface-2/50">
          {annotation}
        </span>
      </div>
      <h2
        ref={titleRef}
        className="text-[1.625rem] md:text-4xl font-medium tracking-[-0.02em] text-text-1 mb-4 overflow-hidden"
      >
        {title}
      </h2>
      <p className="text-text-2 text-[0.8125rem] md:text-sm leading-relaxed mb-6 max-w-[400px]">
        {description}
      </p>
  ...
  if (layout === "full-width") {
    return (
      <section ref={sectionRef} className="relative px-6 overflow-hidden">
        <div className="mx-auto max-w-[1100px]">
          <div className="relative">
            <span
              ref={numberRef}
              className="absolute -top-8 md:-top-16 left-0 text-[clamp(4rem,12vw,14rem)] md:text-[clamp(6rem,15vw,14rem)] font-[100] leading-[0.85] tracking-[-0.05em] text-text-4 pointer-events-none select-none z-0 opacity-0"
            >
              {number}
            </span>
    ...
    <section ref={sectionRef} className="relative px-6 overflow-hidden">
      <div className="mx-auto max-w-[1100px]">
        <div className="relative">
          ...
          <div
            className={`relative z-10 grid gap-6 md:gap-12 ${
              layout === "image-left"
